...
def declare_actions(
    ...
    vision_node = Node(
            ...
            name='vision',
            output='screen',)
    head_moving_node = Node(
            package='tiago_gazebo',
            executable='head_control.py',
            name='head',
            output='screen',)
    ...
    handler2 = RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[head_moving_node]
            )
        )
    handler3 = RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=head_moving_node,
                on_exit=[listener_node, order_node, vision_node]
            )
        )
    launch_description.add_action(handler2)
    launch_description.add_action(handler3)
    return
